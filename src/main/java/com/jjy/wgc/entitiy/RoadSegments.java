...
@ApiModel(value = "RoadSegments对象", description = "定义构成交通网络的有向路段（即图的边），并存储每个路段的静态和半静态属性，这些属性是 WGC 算法进行预测和路径规划的关键输入。")
public class RoadSegments implements Serializable {
     ...
     */
    @ApiModelProperty("Time in seconds for an idle driver to traverse the segment (τe).")
     ...
     */
    @ApiModelProperty("Probability an idle driver at the start node chooses this segment (Q_uv).")
    private Double transitionProbability;

    /**
     * The base rate of passenger requests on this segment (λe).
     */
    @ApiModelProperty("The base rate of passenger requests on this segment (λe).")
     ...
     */
    @ApiModelProperty("PostGIS LineString representing the physical path of the road.")
